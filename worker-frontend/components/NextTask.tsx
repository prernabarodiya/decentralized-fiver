...
export const NextTask = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then(res => setCurrentTask(res.data.task))
  ...
  if (!currentTask) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center text-white/40 text-center px-4">
        No pending tasks right now. Check back later.
      </div>
    );
  }
...
function Option({
  ...
  return (
    <div
      onClick={onSelect}
      className="group cursor-pointer bg-white/5 border border-white/10 rounded-xl overflow-hidden backdrop-blur-md transition hover:-translate-y-1 hover:border-amber-400/40"
    >
      <img
        src={imageUrl}
      ...
      <div className="text-center py-2 text-sm text-white/60 group-hover:text-amber-400 transition">
        Select
      </div>
    </div>
  );
